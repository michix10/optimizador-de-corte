...
import PieceInput from './components/PieceInput.tsx';
import PieceList from './components/PieceList.tsx';
...
import { Piece, Panel, EdgeBanding, Offcut, Rotation } from './types.ts';
import { optimizeCuts } from './services/optimizer.ts';
import { DEFAULT_PANEL_WIDTH, DEFAULT_PANEL_HEIGHT } from './constants.ts';
...
const App: React.FC = () => {
  ...
  const [bandingCost, setBandingCost] = useState(0);
  ...
  const handleOptimize = useCallback(() => {
    ...
    setTimeout(() => {
        try {
            const resultPanels = optimizeCuts(pieces, panelWidth, panelHeight, sawKerf);
  ...
  }, [pieces, sawKerf, panelWidth, panelHeight]);
        ...
        <aside className="w-full lg:w-96 lg:fixed top-16 left-0 lg:h-screen-minus-header p-6 overflow-y-auto bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 no-print">
           <div className="space-y-6">
              ...
              <Configuration
                ...
                bandingCost={bandingCost}
                setBandingCost={setBandingCost}
              />
...
export default App;                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {summary.map((item, index) => (
                            <tr key={index}>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className="font-mono text-sm">
